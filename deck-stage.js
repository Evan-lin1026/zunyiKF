 ...
 *      prev/next (not in mobile browse mode — see (g)) — taps on links,
 *      buttons and other interactive slide content are left alone.
 ...
 *  (g) mobile — on mobile devices the same deck automatically renders as
 *      a scrollable vertical card list (browse) and a rotated fullscreen
 *      presenting mode. This needs NO authoring changes: never hand-roll
 *      mobile/responsive slide layouts. Desktop behaviour is unchanged.
 *
...
(() => {
  ...
  // the product iframe is cross-origin, so the shared helper can't be
  // imported. Deliberately NOT viewport-based: the desktop workbench renders
  // decks in a narrow preview iframe that a width breakpoint would misread
  // as mobile. Keep the regex in sync with design-canvas.jsx's copy.
  ...
  // Mobile visual metrics (values from the platform's mobile design spec).
  const MOB_MARGIN = 12;           // card-stream edge margin
  const MOB_GAP = 8;               // gap between cards
  const MOB_RADIUS = 8;            // card corner radius
  const MOB_BADGE_IDLE_MS = 2000;  // scroll idle before the page badge fades
  const MOB_UI_HIDE_MS = 3000;     // presenting controls auto-hide
  ...
  const stylesheet = `
    ...
    /* ── Mobile browse: vertical card stream ── */
    ...
    /* ── Mobile presenting: CSS pseudo-landscape (no orientation.lock) ──
       Rotate only while the device is physically portrait. Once the user
       turns the phone (system auto-rotate) the viewport itself is landscape,
       and stacking another 90° would render slides sideways — so landscape
       falls back to a plain full-bleed fit. _fitMobilePresent and
       _mobPresentZone branch on the same orientation test. */
    ...
    /* ── Mobile presenting controls (back button + filmstrip) ── */
    .mob-ui { display: none; }
    ...
    .mob-strip {
      ...
      /* Transparent: thumbnails carry their own white frames, and a solid
         strip would blanket the slide behind the overlay. */
      background: transparent;
      overflow-x: auto;
    }
    ...
    .mob-strip .mnum {
      ...
      line-height: 20px;
      font-weight: 500;
      /* The strip floats over the (dark) presenting stage — numbers must be light. */
  ...
  class DeckStage extends HTMLElement {
    ...
    _render() {
      ...
      // Per-slide placement rules for the mobile card stream; rebuilt by _fitMobileList().
    ...
    // The two mobile presentations are mutually exclusive. noscale and
    // _snthumb are capture modes whose output must keep desktop geometry,
    // so they opt out of both.
    _syncMobileMode() {
    ...
    _fitMobileList() {
      ...
      this._mobLastVw = vw; // baseline for _onResize's height-only skip
      const stage = this._canvas.parentElement;
      // Clear the stage inset the desktop _fit() wrote inline.
      ...
      this._slides.forEach((slide, i) => {
        ...
        rules.push(
          ...
          // Radius/border are pre-divided by the scale so the slide's transform
          // lands them at 8px / 0.5px on screen.
      ...
      // Screen-only: these per-slide rules out-specify the print block's
      // ::slotted reset, so unwrapped they would drag the card-stream
      // transforms into the paginated print output.
      ...
      // Badge geometry is fully determined by _mobCards, so badges are laid
      // out directly — no IntersectionObserver needed.
    ...
    _onMobScroll() {
      ...
      // Programmatic scrolls count as scrolling too — either source means
      // "the position is changing", which is what the badge signals.
      ...
      this._mobScrollRaf = requestAnimationFrame(() => {
        ...
        this._mobLastTop = top; // keep tracking even while suppressed
        if (this._mobProgUntil && performance.now() < this._mobProgUntil) {
          // Events are still streaming from a programmatic scroll: extend
          // the window so any animation length is covered. It expires 120ms
          // after the stream stops; user scrolls then resume inference.
          this._mobProgUntil = Math.max(this._mobProgUntil, performance.now() + 120);
          return;
        }
        if (moved < 1) return; // no real displacement (synthetic/terminal event)
        // A pure viewport-centre rule can never select the first/last page:
        // 16:9 cards are far shorter than half a portrait viewport, so their
        // centring targets clamp. Clamp at the extremes, centre in between.
        ...
        if (cur.i !== this._index) {
          this._index = cur.i;
          // Broadcast through the existing core (hash + slide-changed +
          // slidechange). data-deck-active toggling is visually inert here.
    ...
    _mobScrollTo(i, smooth) {
      ...
      // Suppress scroll-position inference while a programmatic scroll is
      // in flight: goto/deep-link already set _index directly, and the
      // centring targets of several edge cards clamp to the same scrollTop,
      // so the position alone cannot tell those pages apart.
      this._mobProgUntil = performance.now() + (smooth ? 800 : 200);
      // Seed the baseline with the target: a scrollTo that lands where the
      // stage already is fires no scroll event, and a null baseline would
      // make the next no-op event read as a real first displacement.
    ...
    _fitMobilePresent() {
      ...
      this._canvas.style.width = this.designWidth + 'px';
      this._canvas.style.height = this.designHeight + 'px';
      // Portrait: the stage is CSS-rotated, so its logical size is the
      // swapped viewport. Landscape (user physically rotated): no rotation,
      // plain fit. The CSS branches on the same orientation query.
    ...
    _onResize() {
      // Card-stream geometry depends only on viewport width, and mobile
      // browser URL bars fire height-only resizes as they collapse/expand —
      // a full refit would rebuild everything and yank the list back to the
      // current page mid-scroll. Width changes (rotate/split) refit as usual.
    ...
    _onTap(e) {
      ...
      // Mobile browse is a scroll list — taps must not navigate.
    ...
    // After rotate(90deg) the visual-landscape x axis maps onto viewport y
    // (direction confirmed on device; if prev/next ever read reversed, flip
    // these comparisons — single point of change).
    _mobPresentZone(e) {
      const rect = this._stage.getBoundingClientRect();
      // Portrait = stage is rotated, visual x comes from viewport y;
      // landscape = no rotation, viewport x directly. Same branch as the fit.
      const portrait = window.innerHeight >= window.innerWidth;
      const x = portrait
    ...
    _toggleMobUi(force) {
      ...
      clearTimeout(this._mobUiTimer);
      if (!on) return;
      // Rebuild only when opening from hidden: the strip is a static
      // snapshot outside _liveObserver's refresh pipeline, so reopening is
      // what absorbs slide edits/reorders/skips. Calls while already open
      // (thumbnail taps resetting the timer) only re-highlight — rebuilding
      // on that hot path would clone every slide per tap.
    ...
    _buildMobUi() {
      // A pure-viewer host may never send rail-enabled, so the author-CSS
      // snapshot (normally built by _enableRail) can be missing and the
      // clones would render unstyled — build it here as a fallback.
      // _snapshotAuthorCss is re-entrant and generation-guarded.
      ...
      this._mobStripEntries = this._slides
        ...
        .map((e2, ordinal) => {
          ...
          t.addEventListener('click', (ev) => {
            ev.stopPropagation();
            this._go(e2.i, 'click');
            this._toggleMobUi(true); // reset the auto-hide timer, keep the overlay up
          });
          strip.appendChild(t);
          // _materialize provides the full clone pipeline (media neutralised,
          // canvas snapshots) and mounts a host into entry.frame itself — but
          // it scales clones by the rail's _thumbScale, so rewrite the
          // transform for this strip's fixed 132px frames.
      ...
      // Clicks inside the overlay must not bubble into tap navigation.
    ...
    _go(i, reason = 'api') {
      ...
      // Mobile browse: navigation sets _index directly (desktop parity) and
      // the scroll is only a visual follow. Inferring the page back from the
      // scroll position is unreliable here — edge cards' centring targets
      // clamp to the same scrollTop.
      if (this.hasAttribute('data-mobile-list')) {
        const changed = clamped !== this._index;
        this._index = clamped;
        // Same-page goto doesn't rebroadcast (desktop parity) but still recentres.
    ...
    _materialize(entry) {
      ...
      // Keep ids: the clone lives in its own shadow root, so duplicate ids
      // can't clash with the document (getElementById doesn't pierce shadow
      // roots), while id-keyed author styles (#slide-01 { background: … })
      // MUST keep matching — stripping ids rendered such decks as blank
      // white thumbnails (styles all failed to match).
